...
Use the TDD skill and existing public interfaces: protected requests/actions, reporting queries over migrated PGlite, management commands, editorial publishing, components/browser journeys and database adapters. These seams were proposed in the recipe and remain to be confirmed before new tests under that skill. Work one failing behavioral test to one minimal implementation at a time. Avoid speculative abstractions and implementation-coupled tests. No runtime tests were added in the decision-persistence session.
...
**Working defaults, not newly accepted user decisions**

Start with one club per installation while modeling club-scoped memberships so another club can be added. Keep authenticated league result comparisons distinct from private club administration. Proposed roles are member, coach and club admin. Invitations should bind to verified email, expire, be single-use and revocable. Proposed squad lifecycle is archive rather than identifier deletion. Exact permission matrix, lifecycle and last-admin behavior require specification.
